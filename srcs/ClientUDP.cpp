...
void clientUDP(std::vector<std::pair<std::string, std::pair<std::string, int>>> &ipList, std::vector<std::pair<std::string, int>> &vizinhos){
    ...
    //::thread printIpListThread(printIpList, std::ref(ipList));
    //printIpListThread.detach();

    close(sockfd);
}

void serverUDP(std::vector<std::pair<std::string, std::pair<std::string, int>>> &ipList, std::vector<std::pair<std::string, int>> &vizinhos){
    int sockfd;
...
int main(){
    ...
    while (std::getline(file, line)) {
        ...
        vizinhos.push_back(make_pair(line, 35));
    }

    file.close();

    std::thread serverThread(serverUDP, std::ref(ipList), std::ref(vizinhos));
    std::this_thread::sleep_for(std::chrono::seconds(1));
    std::thread clientThread(clientUDP, std::ref(ipList), std::ref(vizinhos));
